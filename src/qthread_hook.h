...
class QThread_hook : public QObject_hook {
  ...
  public:
    ...
    void hook_terminated(QHook &hook) { 
      if ( !terminated_event.func )
        connect(handle, SIGNAL(terminated()), this, SLOT(terminated_hook()));
      terminated_event = hook;
      if ( !hook.func )
        disconnect(handle, SIGNAL(terminated()), this, SLOT(terminated_hook()));
    }
  ...
  private slots:
    ...
    void terminated_hook() {
      if ( terminated_event.func ) {
        typedef void (*func_type)(void *data);
	(*(func_type)terminated_event.func)(terminated_event.data);
      }
    }
  private:
    QHook started_event;
    QHook finished_event;
    QHook terminated_event;
};
