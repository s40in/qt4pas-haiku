...
class QActionGroup_hook : public QObject_hook {
  ...
  public:
    ...
    void hook_selected(QHook &hook) { 
      if ( !selected_event.func )
        connect(handle, SIGNAL(selected(QAction*)), this, SLOT(selected_hook(QAction*)));
      selected_event = hook;
      if ( !hook.func )
        disconnect(handle, SIGNAL(selected(QAction*)), this, SLOT(selected_hook(QAction*)));
    }
  ...
  private slots:
    ...
    void selected_hook(QAction* AnonParam1) {
      if ( selected_event.func ) {
        typedef void (*func_type)(void *data, QActionH AnonParam1);
	(*(func_type)selected_event.func)(selected_event.data, (QActionH)AnonParam1);
      }
    }
